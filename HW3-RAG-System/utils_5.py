...
def get_inference_user_prompt(query : str, context_list : List[str]) -> str:
    ...
    prompt = f"""Context Information:\n{context_text}\nQuestion: {query}\n\nInstructions: 1. Copy the exact answer phrase directly from the passages above. 2. Do NOT paraphrase, expand, or add explanations. 3. If multiple phrases answer the question, choose the most direct one. 4. Only output \"CANNOTANSWER\" if no relevant information exists. 5. Your answer must be one continuous text span or CANNOTANSWER. \n\nAnswer:"""
    return prompt
